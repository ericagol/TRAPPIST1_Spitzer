...
lgnu,V1exp2nuinv = x
nu = exp(lgnu)
lnV1 = log(V1exp2nuinv/exp(0.5/nu))
# Normalization is: Gamma((\nu+1)/2)/\sqrt{\nu \pi}/Gamma(\nu/2)/\sigma
# (sigma is factor by which the error bars are inflated - it equals sqrt(exp(lnV1)))
#return -2*log(gamma(0.5*(nu+1))/gamma(0.5*nu))+log(pi*nu)+lnV1
return -2*(lgamma(0.5*(nu+1))-lgamma(0.5*nu))+log(pi*nu)+lnV1
end
...
function lnprob(p::Array{T,1}) where {T <: Real}
  ...
  # lnV1 = log of factor inflating variance foreground
  y, model, yerr, lgndof, V1exp2nuinv = p
  nu = exp(lgndof)
  sig = sqrt(V1exp2nuinv/exp(0.5/nu))
  ll = log_students_t_pdf((y-model)/yerr,sig,nu)
