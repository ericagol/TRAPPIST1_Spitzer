...
for i=1:nplanet
  ...
  print(i," ",size(img))
  if i > 3
    #ax.imshow(img,aspect="auto",cmap="winter",interpolation="nearest",origin="lower",extent = [tmin,tmax,0,1600])
    ax.imshow(img,aspect="auto",cmap="winter",interpolation="nearest",origin="lower",extent = [tmin,tmax,0,1600])
  else
    #ax.imshow(img,aspect="auto",cmap="winter",interpolation="nearest",origin="lower",extent = [tmin/2,tmax/2,0,1600])
    ax.imshow(img,aspect="auto",cmap="winter",interpolation = "nearest",origin="lower",extent = [tmin/2,tmax/2,0,1600])
  end
  ...
  if i == 4 
    ax.set_xlabel(L"Time [hr] - $\mathrm{t_n}$")
  end
  if i == 1
    ax.set_ylabel(L"Time [days] - $\mathrm{t_0}$}")
  else
        ax.set_yticks([])
  end
end
subplots_adjust(hspace=0,wspace=0.05)
savefig("../T1_riverplot.pdf",bbox_inches="tight")
