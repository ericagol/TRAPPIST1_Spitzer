...
photometry includes 188 transits \citep{Ducrot2020}. {In this paper we complement and
...
for which $j P_i^{-1} \approx (j+k) P_{i+1}^{-1}$ for integers $j$ and $k$ for the $i$th and $i+1$th planets.  This proximity causes a resonant timescale {for} $\mathbf{k=1}$
given by
\begin{equation}
...
The current paper is the first with a survey time, $t_{survey} \mathbf{= 4.114}$ {yr}, such that $t_{survey} {>} t_{min}$ for the TRAPPIST-1 system.
...
We provide \texttt{Julia}, \texttt{python}, and \texttt{Matlab} code for running the Markov chains,
{creating the figures,}
and creating the paper PDF in
\texttt{https://github.com/ericagol/TRAPPIST1\_Spitzer}.  
The 3.5 GB \texttt{data/} directory in the repository may be found 
%as a zip file, \texttt{data.zip}, at 
%\texttt{\scriptsize https://drive.google.com/file/d/14iEW6jupY8dnGlYGXJIrWlFftdwRcfLo} and 
at \doi{10.5281/zenodo.4060252}.
...
We {use} a heavy-tailed likelihood function which
...
the same {hemisphere} of the star as shown in \citet{Luger2017b}: the planets'
...
We next searched for variations of density across the planets. For this, {we took} each planetary density calculated from $10^4$ samples {and divided by the density of the closest pair of mass and radius} of a fully differentiated 20 wt\% iron, 80 wt\% silicate (MgSiO$_3$) interior planet with no surface layers, which is less iron rich than Earth.  {A planet with a normalized density of 1 has exactly the same density as the reference model, while a normalized density ${>}1$ (${<}1$) is denser (lighter), than the reference model, respectively.}
...
\begin{figure}
    ...
    {Iron core mass fraction versus the planetary orbital periods {for a fully-differentiated model with molar Mg/Si = 1.02 and no surface layer}.  The approximate values for Earth \citep{McDonough2014}, Mars \citep{Khan2018}, the Moon \citep{Barr2016}, and common chondrites \citep{Palme2014} are indicated, as well as the 1$\sigma$ confidence intervals of the TRAPPIST-1 planets.
...
Fe/Mg molar ratio &  $0.60_{-0.18}^{+0.18}$ & $0.64_{-0.16}^{+0.16}$ & $0.44_{-0.13}^{+0.14}$ & $0.58_{-0.14}^{+0.14}$ & $0.45_{-0.11}^{+0.10}$ & $0.34_{-0.10}^{+0.09}$ & $0.35_{-0.23}^{+0.27}$ & $0.47\pm0.07$ \cr
       \hline
        H$_2$O [wt \%] for: & & & & & & & & \cr
    ...
    \end{tabular}
    \caption{Core mass {fractions and molar Fe/Mg ratio (for a fully-differentiated model), and water mass fractions} inferred for each TRAPPIST-1 planet, as well as the weighted means.}
    \label{tab:cmf}
\end{table*}}
...
\begin{figure}
    ...
    \oscaption{julia/copy_figure18.jl}%
...
Our new samples show a better conservation of the GLR angles than was found in \citet{Grimm2018}, where the longest resonance time was found to be 2 Myr.
...
Julia \citep{Bezanson2017}, Limbdark.jl \citep{Agol2019}.}}
...
\begin{figure}
    ...
    \oscaption{python/plotCorner_ttv.py.script}{Corner plot of variables in the transit-timing analysis with $1\sigma$
...
\begin{figure}
    ...
    \oscaption{python/plotCorner_photdyn.py}{Corner plot of variables in the photodynamical analysis with $1\sigma$
