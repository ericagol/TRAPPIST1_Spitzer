...
given by
\begin{equation}
    P_{TTV} = \frac{1}{j P_i^{-1} - (j+k) P_{i+1}^{-1}},
\end{equation}
\citep{Lithwick2012} which is the characteristic timescale of the transit timing variations (TTVs) \textbf{of
the outer five planets. The inner two pairs of planets vary on a timescale smaller by a factor of 3 (b/c) and 
2 (c/d) thanks to the generalized Laplace resonant conditions.}
