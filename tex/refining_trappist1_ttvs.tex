...
We next searched for variations of density across the planets. For this, \textbf{we took} each planetary density calculated from $10^4$ samples \textbf{and divided by the density of the closest pair of mass and radius} of a fully differentiated 20 wt\% iron, 80 wt\% silicate (MgSiO$_3$) interior planet, which is less iron rich than Earth.  \textbf{A planet with a normalized density of 1 has exactly the same density, while a normalized density ${>}1$ (or ${<}1$, resp.) is denser (or lighter, resp.), than a 20 wt\% iron, 80 wt\% MgSiO$_3$ planet with no surface layer(s).}
